# Skill Call Graph

Generated by `library-skill` on 2026-05-14.

Visual map of how skills in agent-loom call each other. Arrows show direction of invocation (`caller → callee`). Skills with no outgoing arrows are leaf nodes.

```mermaid
flowchart LR
  subgraph Meta
    usc[universal-skill-creator]
    imp[improve-skills]
    val[validate-skills]
    res[research-skill]
    pru[prune-skill]
    com[compress-skill]
    spl[split-skill]
    dep[deprecate-skill]
    lib[library-skill]
    pub[publish-skill]
    gen[generate-changelog]
    lfp[learn-from-paper]
    lfc[learn-from-chat]
    app[apply-paper-to-project]
    sec[secure-skill]
    sec-cs[secure-skill-content-sanitization]
    sec-ri[secure-skill-repo-ingestion]
    sec-rt[secure-skill-runtime]
  end

  subgraph Thinking
    bra[brainstorming]
    inv[inversion]
    adv[adversarial-hat]
    pre[pre-mortem]
    asm[assumption-mapping]
    soc[socratic]
    dt[deep-thinking]
    fp[first-principles]
    so[second-order]
    fer[fermi]
    ooda[ooda]
  end

  subgraph Project-Specific
    ps[product-soul]
    prd[prd-writing]
    ipl[implementation-plan]
    tdd[test-driven-development]
    asa[agent-system-architecture]
    adl[architectural-decision-log]
    tda[technical-debt-audit]
    cbu[codebase-understanding]
    crsp[code-review-crsp]
    daf[debug-and-fix]
    dd[reality-check]
    po[project-orchestrator]
    psu[project-setup]
    rps[retroactive-project-setup]
  end

  subgraph Agent-Process-Design
    pd[process-decomposer]
    aa[agent-builder]
    se[setup-evaluation]
    sf[skill-finder]
    tf[tool-finder]
    cap[create-agent-prompt]
  end

  subgraph Venture-Exploration
    ve[venture-exploration]
    ig[idea-generation]
    bm[business-modeling]
    ie[idea-evaluation]
    cd[customer-discovery]
  end

  %% --- Meta skill call chains ---
  usc --> res
  usc --> spl
  usc --> com
  usc --> val
  usc --> lib
  usc --> pub

  imp --> val
  imp --> dep
  imp --> pru
  imp --> res
  imp --> spl
  imp --> com
  imp --> lib

  com --> spl
  spl --> lib
  dep --> lib
  lib --> gen

  lfp --> sec
  lfp --> usc
  lfp --> app
  lfp --> val

  lfc --> val
  lfc --> imp

  app --> adl

  sf --> usc
  sf --> lib

  %% --- Security chain ---
  sec --> sec-cs
  sec --> sec-ri
  sec --> sec-rt

  %% --- Thinking skill chains ---
  dt --> inv
  dt --> pre
  dt --> asm
  dt --> soc
  dt --> adv
  dt --> fp
  dt --> so
  dt --> fer
  dt --> ooda

  bra --> inv
  prd --> inv
  ps --> inv

  %% --- Process & agent design chains ---
  pd --> sf
  pd --> tf
  pd --> res

  aa --> asa
  aa --> cap
  aa --> se
  aa --> po

  %% --- Due diligence ---
  dd --> adv
  dd --> asm
  dd --> cbu
  dd --> ipl

  %% --- Orchestrator ---
  po --> psu

  %% --- Retroactive project setup ---
  rps --> cbu
  rps --> ps
  rps --> adl
  rps --> psu
  %% rps invokes adl with SYNTHESIS=true, psu with RETROACTIVE=true

  %% --- Venture exploration suite ---
  ve --> ig
  ve --> bm
  ve --> ie
  ve --> cd
  ie --> fer
  ie --> asm
  ie --> pre
  ie --> adv
  bm --> fer
  cd --> sec
  ig --> fp
  ig --> soc
  ve --> ps
```

## Reading the Graph

- **Entry points** — skills users invoke directly: `universal-skill-creator`, `improve-skills`, `learn-from-paper`, `reality-check`, `project-orchestrator`, `project-setup`, `retroactive-project-setup`, `process-decomposer`, `agent-builder`, `deep-thinking`, `brainstorming`, `prd-writing`, `product-soul`, `venture-exploration`
- **Meta chain** — `improve-skills` runs the full cycle: validate → ingest `docs/learnings/chat-learnings.md` → deprecate → prune → research → rewrite → split/compress → cross-link → library-skill → generate-changelog → write terminal statuses back to chat-learnings
- **Targeted improvement loop** — `learn-from-chat` (in-session) escalates restructure-class edits to `improve-skills TARGET=<skill> SKIP_RESEARCH=true`; `improve-skills` also reads OPEN entries from `docs/learnings/chat-learnings.md` during periodic full passes — the two skills form one closed feedback loop
- **Retroactive backfill** — `retroactive-project-setup` invokes `architectural-decision-log SYNTHESIS=true` (inferred ADR backfill) and `project-setup RETROACTIVE=true` (AGENTS.md emit without interview); both modes mark inferred content honestly so future agents don't mistake reconstruction for live capture
- **Self-enforcing checkpoints** — `validate-skills` Step 4c audits every producer skill against the `memory/SKILL.md` checkpoint registry; missing registrations surface as structural flags rather than relying on humans to notice
- **Security gate** — `secure-skill` orchestrates three sibling skills (`content-sanitization`, `repo-ingestion`, `runtime`) and is called by `learn-from-paper` and internally by `research-skill`, `universal-skill-creator`, `improve-skills`
- **Process layer** — `process-decomposer` → `agent-builder` → `setup-evaluation` → `project-orchestrator` → execution
- **Leaf nodes** (called but call nothing): `validate-skills`, `research-skill`, `prune-skill`, `publish-skill`, `generate-changelog`, `tool-finder`, `create-agent-prompt`, `setup-evaluation`, and all thinking framework skills except `deep-thinking`
